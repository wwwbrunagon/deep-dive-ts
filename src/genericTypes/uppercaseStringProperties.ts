...
type UppercaseStringProperties<T> = {
   ...
   * @returns {UppercaseStringProperties<T>[]} - A new array with the transformed objects where string properties are in uppercase.
   * 
   * @example
   ...
   * // ]
   */
  function transformStringsToUppercase<T>(data: T[]): UppercaseStringProperties<T>[] {
    return data.map(item => {
      const transformedItem = {} as UppercaseStringProperties<T>;
      for (const key in item) {
        if (typeof item[key] === 'string') {
          // Convert string properties to uppercase
          transformedItem[key] = (item[key] as string).toUpperCase() as UppercaseStringProperties<T>[typeof key];
        } else {
          // Preserve non-string properties
          transformedItem[key] = item[key] as UppercaseStringProperties<T>[typeof key];
        }
      }
